        ...
        if echo "$status" | grep -q "NotReady"; then
            ...
            if [[ -z "${failed_nodes[$node]}" ]]; then
                ...
                if [[ $elapsed -ge 900 ]]; then
                    echo "15 minutes elapsed, force removing pods from $node."
                    kubectl get pods --all-namespaces --field-selector spec.nodeName==$node --no-headers | awk '{print $2 " " $1}' | while read -r pod namespace; do
            ...
            # If node recovers, remove it from failed tracking
            if [[ -n "${failed_nodes[$node]}" ]]; then
