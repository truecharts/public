...
data:
  ACCESS_URL: {{ if .Values.ingress.main.enabled }}{{ with (first .Values.ingress.main.hosts) }}https://{{ .host }}{{ end }}{{ else }}http://{{ .Values.env.AccessIP | default ( printf "%v-%v" .Release.Name "nextcloud" ) }}:{{ .Values.service.main.ports.main.port }}{{ end }}
