...
{{- if ne "disabled" .Values.addons.vpn.type -}}
  ...
  {{- if .Values.addons.vpn.config -}}
    ...
    {{- $secret := include "tc.v1.common.addon.openvpn.secret" . | fromYaml -}}
    {{- if $secret -}}
  ...
  {{- if or .Values.addons.vpn.configFile .Values.addons.vpn.config .Values.addons.vpn.existingSecret -}}
    ...
    {{- $configper := include "tc.v1.common.addon.openvpn.volume.config" . | fromYaml -}}
    {{- if $configper -}}
  ...
  {{- range $targetSelector -}}
    ...
    {{- $containerModify := dict -}}
      ...
      {{- $containerModify = include "tc.v1.common.addon.openvpn.containerModify" $ | fromYaml -}}
