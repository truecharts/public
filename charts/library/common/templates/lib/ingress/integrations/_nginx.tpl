...
{{- define "tc.v1.common.lib.ingress.integration.nginx.validate" -}}
  ...
  {{- if $nginx.themePark }}
  {{- if $nginx.themePark.enabled }}
  {{- if not (kindIs "string"  $nginx.themePark.css) -}}
    {{- fail (printf "Ingress - Expected [integrations.nginx.themepark.css] to be a [string], but got [%s]" (kindOf $nginx.certificateIssuer)) -}}
  {{- end -}}
  {{- end -}}
  {{- end -}}
