!(function (e, r) {
  ...
  return function (e) {
    ...
    (e.hi = function () {
      this.pipeline.reset(),
        this.pipeline.add(e.hi.trimmer, e.hi.stopWordFilter, e.hi.stemmer),
        this.searchPipeline &&
          (this.searchPipeline.reset(), this.searchPipeline.add(e.hi.stemmer));
    }),
      (e.hi.wordCharacters =
        "ऀ-ःऄ-एऐ-टठ-यर-िी-ॏॐ-य़ॠ-९॰-ॿa-zA-Zａ-ｚＡ-Ｚ0-9０-９"),
      (e.hi.trimmer = e.trimmerSupport.generateTrimmer(e.hi.wordCharacters)),
      e.Pipeline.registerFunction(e.hi.trimmer, "trimmer-hi"),
      (e.hi.stopWordFilter = e.generateStopWordFilter(
    ...
    var r = e.wordcut;
    r.init(),
      (e.hi.tokenizer = function (i) {
        if (!arguments.length || null == i || void 0 == i) return [];
        if (Array.isArray(i))
          return i.map(function (r) {
            return isLunr2 ? new e.Token(r.toLowerCase()) : r.toLowerCase();
          });
        var t = i.toString().toLowerCase().replace(/^\s+/, "");
        return r.cut(t).split("|");
      }),
      e.Pipeline.registerFunction(e.hi.stemmer, "stemmer-hi"),
      e.Pipeline.registerFunction(e.hi.stopWordFilter, "stopWordFilter-hi");
  };
});
