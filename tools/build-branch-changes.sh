...
main() {
    ...
    local owner=discretizer
    local repo=charts
    local production=
    local charts_repo_url=
    local token=$(gh auth token)
    local parthreads=$(($(nproc) * 2))
    local testing_branch=$(git rev-parse --abbrev-ref HEAD)
    ...
    echo "Discovering changed charts in branch '$testing_branch'..."
    local changed_charts=()
    readarray -t changed_charts <<< "$(lookup_changed_charts $base_branch)"
...
validate_catalog() {
    ...
    docker run -ti -v ${PWD}:/test docker.io/ixsystems/catalog_validation /usr/local/bin/catalog_validate validate --path "/test/catalog"
    fi
}
...
upload_catalog() {
    ...
    cd catalog
    git config user.name "Discretizer Test Uploader"
    git config user.email "zenoftheblues@gmail.com"
    git add --all
...
filter_charts() {
    while read -r chart; do
        [[ ! -d "$chart" ]] && continue
        if [[ $(git diff master $chart/Chart.yaml | grep "+version") ]]; then
            echo "$chart"
        else
...
lookup_changed_charts() {
    ...
    local changed_files
    changed_files=$(git diff --find-renames --name-only "$base_branch" -- "$charts_dir" | grep "Chart.yaml")
    echo $changed_files

    local depth=$(( $(tr "/" "\n" <<< "$charts_dir" | sed '/^\(\.\)*$/d' | wc -l) + 1 ))
    local fields="1-${depth}"

    cut -d '/' -f "$fields" <<< "$changed_files" | uniq | filter_charts $base_branch
}
